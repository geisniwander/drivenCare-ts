...
async function findAppointmentsByDate(
  doctor_id: Number,
  date: Date,
  time: string
): Promise<QueryResult> {
...
async function findAppointmentsById(
...
async function findAppointmentsByDoctorId(id: Number): Promise<QueryResult> {
  const result = await connectionDb.query(
...
async function findAppointmentsByPatientId(id: Number): Promise<QueryResult> {
  const result = await connectionDb.query(
    `SELECT "public.appointments".id, "public.doctors".name AS doctor_name, "public.patients".name AS patient_name, "public.doctors".specialty, "public.appointments".date, "public.appointments".canceled
        ,"public.appointments".finished
        FROM "public.appointments"
